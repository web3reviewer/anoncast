import { BEST_OF_FID } from '../src/utils'
import { Cast, GetCastsResponse } from '../src/services/types'
import {
  bytesToHexString,
...
async function main() {
  const messages = await getCastMessages(BEST_OF_FID)
  ...
  await redis.set(`trending:${BEST_OF_FID}`, JSON.stringify(sortedCasts))

  console.log(`Updated trending for ${BEST_OF_FID}`)
}
